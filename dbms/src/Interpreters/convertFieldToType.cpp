    ...
    else if (const DataTypeTuple * type_tuple = typeid_cast<const DataTypeTuple *>(&type))
    {
        if (src.getType() == Field::Types::Tuple)
        {
            ...
            for (size_t i = 0; i < dst_tuple_size; ++i)
            {
                 ...
                 *
                 * This means that for conversion to Tuple(Tuple(UInt8)), each
                 * of 'tuple(tuple(-1))', 'tuple(null)' and 'null' will give the
                 * same Null. It might hide some user errors, but there is no
                 * easy way to detect them, given that the signature of
                 * convertFieldToType doesn't distinguish between explicit Nulls
                 * and Nulls due to unconvertible value. This might need to be
